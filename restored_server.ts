...
async function startServer() {
  ...
  app.post('/api/principles', requireAuth, async (req, res) => {
    ...
    try {
      ...
      console.error(e);
      res.json({ principles: ['Foundation', 'Core Concepts', 'Advanced Mechanics', 'Practical Application', 'Mastery'] });
    }
  });
  ...
  app.post('/api/principle-test', requireAuth, async (req, res) => {
    ...
    try {
      ...
      res.json({ question: 'Fallback question: Explain the topic.' });
    }
  });
  ...
  app.post('/api/curriculum', requireAuth, async (req: AuthRequest, res) => {
    ...
    try {
      ...
      console.error(error);
      return res.json({
        title: `${skill} (Fallback Mode)`,
        description: "This is a fallback curriculum generated because the AI service is currently unavailable.",
        modules: [
          {
            id: "m1",
            title: "Module 1: Getting Started",
            lessons: [
              { id: "m1l1", title: "Introduction" },
              { id: "m1l2", title: "Basic Concepts" }
            ]
          },
          {
            id: "m2",
            ...
            lessons: [
              { id: "m2l1", title: "Deep Dive" },
              { id: "m2l2", title: "Practical Examples" }
            ]
          }
        ]
      });
...
ABSOLUTELY NO technical jargon. Explain everything as simply as possible.`
      ...
      console.error(error);
      return res.json({
        content: `### ${lessonTitle}\n\n**Note: AI Service Unavailable.**\n\nThis is fallback text because the AI service encountered an error.\n\nIn a real scenario, this section would contain a detailed, highly-structured lesson about ${lessonTitle}, customized to your learning style, including practical code snippets and theoretical deep-dives.`
      });
    }
  });
  ...
  app.post('/api/chat', requireAuth, async (req: AuthRequest, res) => {
    ...
    try {
      ...
      const content = await generateAIResponse(messages, { groq: 'llama-3.1-8b-instant' });

      res.json({ text: content || '' });
    } catch (error: any) {
      console.error(error);
      return res.json({
        text: "I'm sorry, but the AI service is currently unavailable. I am operating in a limited fallback mode."
      });
    }
  });
  ...
  app.post('/api/ask-highlight', requireAuth, async (req: AuthRequest, res) => {
    ...
    try {
      const content = await generateAIResponse([
          {
            role: 'system',
            content: 'You are an elite AI tutor. Answer questions about the highlighted text VERY simply. Use simple words and analogies. NO technical jargon.'
          },
          {
            role: 'user',
      ...
      console.error(error);
      return res.json({
        answer: "I couldn't process your highlight question due to an error. Please try again."
      });
    }
  });
  ...
  app.post('/api/generate-test', requireAuth, async (req: AuthRequest, res) => {
    ...
    try {
      ...
      res.json({ question: 'Fallback question: Explain the topic in depth.' });
    }
  });
...
Evaluate strictly.`
      ...
      res.json({ passed: true, feedback: 'Passed via fallback.' });
    }
  });
  ...
  app.post('/api/generate-sandbox', requireAuth, async (req: AuthRequest, res) => {
    ...
    try {
      ...
      res.json({ task: 'Fallback Sandbox Task: Create a comprehensive real-world project.' });
    }
  });
...
Verify this strictly. Extract 3 specialities.`
      ...
      res.json({ verified: true, feedback: 'Verified via fallback.', specialities: ['Resilience', 'Problem Solving', 'Adaptability'] });
    }
  });
